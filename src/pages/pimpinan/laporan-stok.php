...
<main x-data="container">
    <div class="px-4 pt-6">
        <div class="mb-4 col-span-full">
            <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white" ref="title">Buat Laporan Stok</h1>
        </div>
        <?php if(session('temp')): ?>
        ...
        <div class="px-4 py-8 mb-4 bg-white border border-gray-200 rounded-lg shadow-sm w-1/2">
            <form @submit.prevent="unduhLaporan">
                <div class="mb-4">
                    ...
                    <select x-model="properties.form.kriteria" class="cursor-pointer shadow-sm bg-gray-50 border-2 border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:border-gray-500 outline-none block w-full p-2.5">
                        <option value="1">-- Semua Stok Beras --</option>
                        <option value="2">-- Hanya Stok Tersedia --</option>
                        <option value="3">-- Hanya Stok Kosong --</option>
                    </select>
                </div>
                <div>
